...
class TestScene:
    """Test the Scene dataclass and build_scene function."""

    def test_build_scene_basic(self):
    ...
    def test_build_scene_deterministic_layout(self):
    ...
    def test_build_scene_different_layouts(self):
    ...
    def test_build_scene_size_calculation(self):
...
class TestViewModel:
    """Test the ViewModel dataclass and compute_viewmodel function."""

    def test_compute_viewmodel_basic(self):
    ...
    def test_compute_viewmodel_stats(self):
    ...
    def test_compute_viewmodel_empty_beliefs(self):
    ...
    def test_compute_viewmodel_missing_claim(self):
    ...
    def test_compute_viewmodel_with_telemetry_row(self):
        """Test viewmodel computation with telemetry row (currently unused)."""
        world = _build_test_world(2, 1)
        scene = build_scene(world, layout_seed=42)
        snapshot = _build_test_snapshot(world, tick=1)
        ...
        telemetry_row = TelemetryRow(
            ...
            mean_abs_delta=0.05,
            max_abs_delta=0.1,
            mean_abs_error_to_truth=0.3,
            max_abs_error_to_truth=0.4,
            fraction_truth_aligned=0.7,
            num_observations=2,
            num_verifications=1,
            num_communicate_edges=1,
            num_broadcast_edges=2,
            num_agent_updates=3,
            step_runtime_ms=10.5,
        )
        ...
        # Telemetry row is currently unused, but should not cause errors
        assert vm.tick == 1
        assert vm.claim_id == 0

    def test_compute_viewmodel_duplicate_edge_removal(self):
...
class TestVizIntegration:
    ...
    def test_network_viz_initialization(self, mock_plt):
    ...
    @patch("simlab.viz.network_viz.plt")
    def test_run_viz_basic(self, mock_plt):
    ...
    def test_run_viz_validation(self):
...
class TestVizComponents:
    """Test visualization components (if accessible)."""

    @patch("simlab.viz.network_viz.plt")
    def test_component_structure(self, mock_plt):
